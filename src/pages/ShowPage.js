...
const ShowPage = () => {
    ...
    const [{ state: {similarShows, currentPageSimilar, totalPagesSimilar}, loadingSimilar, errorSimilar}, fetchSimilarShows] = useSimilarShowsFetch(showId);
    const [{ state: {recommendedShows, currentPageRec, totalPagesRec}, loadingRec, errorRec}, fetchRecommendedShows] = useRecommendedShowsFetch(showId);
    ...
    const loadMoreSimilarShows = () => {
        const endpoint = `${API_URL}tv/${showId}/similar?api_key=${API_KEY}&page=${currentPageSimilar + 1}`;
